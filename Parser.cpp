...
Formula* Parser::parseFormula(std::string formula)
{
	...
	for (int i = int(formula.size()) - 1; i >= 1; i--)
	{
		if (formula[i] == '-' && formula[i + 1] == '>' && formula[i - 1] != '<')
		{
			...
			int child1FormulaIndex = getFormulaIndex(formula, start1);
			int child2FormulaIndex = getFormulaIndex(formula, start2);
			int formulaIndex = formulas.size() + 1;

			formulas.push_back(new Negation);
			formulas.push_back(new Disjunction);
			((Negation*)formulas[formulaIndex - 1])->child = formulas[child1FormulaIndex];
			((Disjunction*)formulas.back())->children.push_back(formulas[formulaIndex - 1]);
			((Disjunction*)formulas.back())->children.push_back(formulas[child2FormulaIndex]);
...
std::vector<Formula*> Parser::parseInput(std::string input)
{
	int isRightSide = 0;
